...
func TestParseFile_MatrixValueStringification(t *testing.T) {
	// Values are the literal scalar text: numbers are never reformatted,
	// bools stay their YAML spelling, quoted strings keep their content.
	path := writeTempDats(t, `
tests:
  - cmd: echo "{matrix.i} {matrix.f} {matrix.b} {matrix.s}"
    matrix:
      ...
      f: [1.50]
      b: [true, false]
      s: ["quoted text", 'single']
`)
	tf, err := ParseFile(path)
	require.Nil(t, err)
	assert.Equal(t, Matrix{
		{Name: "i", Values: []string{"1", "42"}},
		{Name: "f", Values: []string{"1.50"}},
...
func TestParseFile_MatrixNullTreatedAsAbsent(t *testing.T) {
	// `matrix:` with an explicit null value never reaches Matrix's
	// unmarshaler (yaml.v3 leaves the field at its zero value for null
	// nodes), so it is indistinguishable from an absent key -- the same
	// behavior `shared:` has. This test pins that choice.
	path := writeTempDats(t, `
tests:
  - cmd: echo hi
    matrix:
...
tests:
  - cmd: echo hi
    matrix:
      x: [a]
      x: [b]
`, `matrix variable "x" declared more than once`},
		"empty value list": {`
tests:
  - cmd: echo hi
    matrix:
...
tests:
  - cmd: echo hi
    matrix:
      x: [1.50, "1.50"]
`, `matrix variable "x" lists duplicate value "1.50"`},
	}
