...
| `Jobs` | `0` (serial) | run up to N commands concurrently, as `-jN` does |
...
weaker thing. Individual files can still narrow this (`sandbox: false`, or an
`image:`) — a file can only ever narrow what the caller allowed.
