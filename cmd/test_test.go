...
func TestRunTestsInvalidFile(t *testing.T) {
	var out bytes.Buffer
	err := runTests(context.Background(), []string{"/nonexistent/test.dats"}, &out, 0)
	assert.NotNil(t, err)
...
`
	...
	var out bytes.Buffer
	err := runTests(context.Background(), []string{f1, f2}, &out, 0)
	assert.Nil(t, err)
...
func TestRunTestsMultipleFilesFailure(t *testing.T) {
	...
	var out bytes.Buffer
	err := runTests(context.Background(), []string{pass, fail}, &out, 0)
	assert.ErrorIs(t, err, errTestsFailed)
