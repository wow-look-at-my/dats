...
# Sandboxing is ON by default (auto: bwrap, then docker). Opt out per run:
...
- `runner/` - Native test runner (public, importable by external modules). RunFile/RunTest/RunFilesParallel/Execute take a context: cancellation kills in-flight process groups (surfacing as signal deaths, never as timeouts); teardown runs under context.WithoutCancel so it always executes
  ...
  - `sandbox.go` - Sandboxing: `SandboxMode`/`SandboxConfig` (mode + docker image; `Backend()` memoizes detection behind a sync.Once, so the host is probed at most once per process and only when a file needs a sandbox), the two probes (both EXERCISE the backend — bwrap is often installed on kernels denying it a userns, the docker CLI often has no daemon), `newSandboxPlan` (CLI choice is the outer bound; a file's spec can only narrow it), and the argv builders. bwrap: `--ro-bind / /` + `--dev`/`--proc`/`--tmpfs /tmp` + `--unshare-pid --die-with-parent`, then per-file `--bind`s (ORDER IS LOAD-BEARING: the tmpfs must precede a work dir under /tmp), `--unshare-net` when network is off. docker: `--rm -i --init --name <n>` + `--user` + rw bind of the work dir (targets deduplicated, rw wins) + ro bind of the cwd + `-e` for dats-added env only, ending in `image bash -c cmd`; the returned `Kill` hook `docker kill`s the container, since killing the client would leave the workload running
