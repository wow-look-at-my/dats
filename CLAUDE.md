...
- **SandboxSpec** - The file-level `sandbox` key: a map of `network`/`image` only (there is deliberately no `writable` key -- scratch goes in the temp dir -- and no `enabled` key: a file narrows its sandbox, never turns it off). The `Network` pointer keeps "unstated" distinct from an explicit `false`; nil spec = nothing narrowed. `sandbox: false`/`enabled:` (both spellings of off) are parse errors naming `--no-sandbox`, and so is `sandbox: true`; unknown/duplicate keys, non-bool `network`, empty `image`, a non-mapping value, and an empty mapping are too. `{matrix.X}` is rejected in `image` (the sandbox is resolved once per file, before instances exist)
