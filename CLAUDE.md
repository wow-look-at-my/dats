...
- `windows-sandbox` - the docker backend on an NT host, where the daemon runs under WSL: it runs `.github/scripts/install-sandbox-backend.sh` (the Windows branch installs WSL, docker.io and a TCP daemon, because the runner's own daemon serves WINDOWS containers) and then this commit's APE over `examples/sandbox.dats`, whose read-only-cwd and writable-temp tests are exactly the binds the daemon has to resolve. A bind it cannot resolve mounts an EMPTY directory instead of failing, so nothing but a real run catches it
- `windows` - the NT path spelling, on an NT host: it runs THIS commit's APE (the `test` job's `go-build` hand-off) over `examples/example.dats` with `--no-sandbox`. Without it the conversion in `runner/hostpath.go` is only asserted by unit tests forcing `hostGOOS`, and a real NT run is where a missed call site shows up
