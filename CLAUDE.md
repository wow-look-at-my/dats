...
5. Command is executed via `bash -c` — inside the file's sandbox unless the run or the file opted out (`runner/sandbox.go`; the wrapper ends in the same `bash -c`) — with placeholder expansion
...
- `cmd/` - Cobra CLI commands (each command self-registers in its own file)
  ...
  - `sandbox.go` - The `--sandbox` (auto|bwrap|seatbelt|docker|none, default auto), `--no-sandbox` (alias for none; contradicting `--sandbox` is an error), and `--sandbox-image` flags, plus `resolveSandbox` → `*runner.SandboxConfig` (nil = opted out). Nothing is probed here — resolution is lazy, so a corpus whose files all declare `sandbox: false` runs with no backend installed
  - `test.go` - `test` subcommand (also the default action): runs tests; jobs==0 runs the serial loop, jobs>=1 calls `runner.RunFilesParallel`; after totals (and the goldens summary under `--update`) it writes any requested report files (a write failure is a real error even when tests passed). runTests takes a context (plain `dats test` passes context.Background() — no signal handling, behavior unchanged; `dats watch` passes its interrupt context)
...
- `schema/` - YAML schema types + parser (public, importable by external modules)
  ...
  - `sandbox.go` - `SandboxSpec`: the file-level `sandbox` key (scalar bool or mapping of `enabled`/`network`/`image`/`writable`, strictly validated — unknown/duplicate keys, wrong types, and an empty mapping are parse errors) plus the nil-safe `IsEnabled`/`NetworkEnabled` accessors (unstated = sandboxed, network on)
...
- **SandboxSpec** - The file-level `sandbox` key: scalar bool (`false` = opt this file out) or a map of `enabled`/`network`/`image`/`writable`. Pointer fields keep "unstated" distinct from an explicit `false`; nil spec = the CLI decides. Unknown/duplicate keys, non-bool `enabled`/`network`, empty `image`, empty `writable`, and an empty mapping are parse errors; `{matrix.X}` is rejected in `image`/`writable` (the sandbox is resolved once per file, before instances exist)
...
sandbox: false              # Optional: opt this file's commands out of the sandbox
                            # (or a map: enabled/network/image/writable)
...
| `sandbox` | No | `false` opts this file's commands (tests AND setup/teardown) out of the sandbox; a mapping (`enabled`, `network`, `image`, `writable`) narrows it. File-level only -- one file's commands share one temp dir and one hook lifecycle. The CLI is the outer bound: under `--no-sandbox` the block is inert, and `sandbox: true` cannot force a sandbox onto an opted-out run |
