...
- `no-tests-in-yaml` - fails when a `run:` script holds a test rather than invoking a suite (`wow-look-at-my/actions@no-tests-in-yaml#latest`). Its own job on purpose: the `test` job already gets the same guard through `go-toolchain@v1`, so keeping it separate means the rule still runs when that job does not
