...
- `docs/` - Additional prose documentation (`library.md` = the Go API and its contracts; `reports.md` = report formats + stability contract; `sandbox-internals.md` = the backends' argv builders and why each bind exists; `sandbox-masked-proc.md` = why a container refuses the sandbox a private procfs, and the read-only-bind fallback that keeps every containment property); `schema.json` - JSON Schema for IDE validation
